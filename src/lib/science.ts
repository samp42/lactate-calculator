import {
  ZoneCalculationMethods,
  type RampTest,
  type RampTestStage,
  type Thresholds,
} from './models'

import polynomial_regression from './math'
...
export function calculateZones(
  ...
  if (test.stages.length < 4) {
    throw Error('At least 4 stages are required to calculate zones')
  }
  ...
  switch (method) {
    ...
    case ZoneCalculationMethods.MODIFIED_DMAX:
      return {}
    case ZoneCalculationMethods.LOG_LOG_DMAX:
      return {}
...
export function calculateKeyMetrics(test: RampTest, method: ZoneCalculationMethods): KeyMetrics {
  return {
